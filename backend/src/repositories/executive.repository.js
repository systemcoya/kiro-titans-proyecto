...
/**
 * Get current month total spend (AI + MegaBill).
...
/**
 * Get previous month total spend.
 * @returns {Promise<number>} Total spend in USD
 */
const getPreviousMonthSpend = async () => {
  const sql = `
    SELECT
...
/**
 * Get top 5 consumers by spend in current month.
 * @returns {Promise<Array<{name: string, spendCop: number}>>}
 */
const getTop5Consumers = async () => {
  const sql = `
...
/**
 * Get average cost per transaction across all use cases.
 * @returns {Promise<number>}
 */
const getAverageCostPerTransaction = async () => {
  const sql = `
    SELECT
      CASE
